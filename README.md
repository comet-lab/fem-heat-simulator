...
To perform the spatial discretization, run the function 'FEM_Simulator::createKMFelem()' and perform the time stepping using
...
To set the fluence rate of the laser, use the method 'FEM_Simulator::setNFR()'. The fluence rate at each node
...
approximate it. In this case, it may be appropriate to use the ``layerHeight`` and ``layerSize`` attributes. 
...
- ``layerSize``: the number of elements that will be used to to represent the first layer
