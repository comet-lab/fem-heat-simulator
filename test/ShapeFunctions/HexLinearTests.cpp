...
TEST(HexLinear, testdN_face)
{
	...
	std::array<float, 2> gp5 = { -1.0f,1.0f };
	int faceIdx = 0;
	std::array<std::array<float, 2>, 4> point2True = { {{-0.5f,-0.5f},{-0.5f,0.0f},{0.0f,-0.5f},{0.0f,0.0f} } };
	std::array<std::array<float, 2>, 4> point3True = { {{0.5f,0.0f},{0.5f,-0.5f},{0.0f,0.0f},{0.0f,0.5f} } };
	std::array<std::array<float, 2>, 4> point4True = { {{0.0f,0.0f},{0.0f,0.5f},{0.5f,0.0f},{0.5f,0.5f} } };
	std::array<std::array<float, 2>, 4> point5True = { {{0.0f,0.5f},{0.0f,0.0f},{-0.5f,0.5f},{-0.5f,0.0f} } };
	...
	*/
	for (faceIdx = 0; faceIdx < 6; faceIdx++)
	{
		...
		{
			std::cout << "Face: " << faceIdx << " A: " << A << std::endl;
			//Eigen::Vector2f output = testFunctions.dNdxi_face(gp1, A, faceIdx);
			...
			Eigen::Vector2f output5 = testFunctions.dNdxi_face(gp5, A, faceIdx);
			//EXPECT_FLOAT_EQ(1 / 4.0f, output[0]);
			for (int idx = 0; idx < 2; idx++)
			{
				...
				EXPECT_FLOAT_EQ(point5True[A][idx], output5[idx]);
			}
		}
	}
}